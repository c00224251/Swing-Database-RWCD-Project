...
class Gui implements ActionListener {
	...
	// Main Stuff to Edit
	final static String projectName = "CA3 Project"; // Project Name
	final static int[] minDimension = { 500, 200 }; // Set dimensions
	final static String[] mainMenu = { "Main Menu", "Customer", "Product", "Invoice" };// All the pages
	final static String[] buttons = { "Add", "Delete", "Edit", "Refresh" };
	...
	static JScrollPane scrollCustomers = new JScrollPane(customers);
	static JScrollPane scrollProducts = new JScrollPane(products);
	static JScrollPane scrollInvoices = new JScrollPane(invoices);
	...
	private JPanel addContent(JPanel panel, int i) {
		...
		if (i == 1) { // Customers
			scrollCustomers.setBackground(color3);
			JPanel options = new JPanel();
			options = addButtons(options, "Customer");
			panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
			panel.add(scrollCustomers, BorderLayout.CENTER);
			panel.add(options, BorderLayout.PAGE_END);
		}
		if (i == 2) { // Products
			scrollProducts.setBackground(color3);
			JPanel options = new JPanel();
			options = addButtons(options, "Product");

			panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
			panel.add(scrollProducts, BorderLayout.CENTER);
			panel.add(options, BorderLayout.PAGE_END);
		}
		if (i == 3) { // Invoices
			scrollInvoices.setBackground(color3);
			JPanel options = new JPanel();
			options = addButtons(options, "Invoice");
			panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
			panel.add(scrollInvoices, BorderLayout.CENTER);
			panel.add(options, BorderLayout.PAGE_END);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (buttonLabel.equals("Add Customer")) {
			...
			Popup.information("Works!");
		} else if (buttonLabel.equals("Refresh Invoice")) {
			refresh();
			Popup.information("Refreshing");
		} else if (buttonLabel.equals("Refresh Customer")) {
			refresh();
			Popup.information("Refreshing");
		} else if (buttonLabel.equals("Refresh Product")) {
			refresh();
			Popup.information("Refreshing");
		}
	}
	...
	public void refresh() {
		scrollCustomers.revalidate();
		scrollCustomers.repaint();
		scrollInvoices.revalidate();
		scrollInvoices.repaint();
		scrollProducts.revalidate();
		scrollProducts.repaint();
	}
